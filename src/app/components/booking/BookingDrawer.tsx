...
function getRandomGreenShade() {
	const hue = 105 + Math.floor(Math.random() * 35);
			...
			<div className="rounded-3xl border border-[#82a687]/30 bg-gradient-to-br from-[#f2fbf6]/90 via-white/60 to-[#e6f4ec]/60 p-5 shadow-[0_20px_60px_rgba(64,93,63,0.14)]">
				<div className="space-y-3">
					<input
						...
						className={`w-full rounded-2xl border border-gray-300/80 bg-white/80 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#82a687] ${playfairRegular.className}`}
					/>
...
export default function BookingDrawer() {
	...
	const canSubmit =
		...
		customerEmail.trim() &&
		paymentMethod;
	const [hoverColors, setHoverColors] = React.useState<Record<string, string>>({});

	const setRandomHoverColor = React.useCallback((key: string) => {
		setHoverColors((prev) => ({
			...prev,
			[key]: getRandomGreenShade(),
		}));
	}, []);

	const clearHoverColor = React.useCallback((key: string) => {
		setHoverColors((prev) => {
			...
			{isOpen && (
				<>
					...
					<motion.aside
						...
						className={`fixed right-0 top-0 z-[80] h-dvh w-full sm:max-w-xl lg:max-w-2xl bg-[#f6fbf8] shadow-[-10px_0_40px_rgba(0,0,0,0.16)] overflow-y-auto ${playfairRegular.className}`}
					>
						<div className="sticky top-0 z-10 border-b border-[#82a687]/20 bg-[#f6fbf8]/90 backdrop-blur-md px-5 py-4">
							<div className="flex items-center justify-between gap-4">
								<div>
									<p className={`text-[42px] leading-none text-[#2c3e50] ${cookie.className}`}>
										Book Now
									</p>
									<p className={`mt-1 text-sm text-gray-600 ${playfairRegular.className}`}>
								...
								<button
									type="button"
									onClick={closeBooking}
									className="rounded-full border border-[#82a687]/30 bg-white/80 px-4 py-2 text-sm hover:cursor-pointer hover:bg-gray-100"
								>
							...
							{step === 1 && (
								<div className="space-y-6">
									<div className="rounded-3xl border border-[#82a687]/30 bg-gradient-to-br from-[#f2fbf6]/90 via-white/50 to-[#e6f4ec]/60 p-5 shadow-[0_20px_60px_rgba(64,93,63,0.14)]">
										<p className={`text-lg text-gray-800 underline ${playfairBold.className}`}>
														...
														>
															<div className="flex items-start justify-between gap-4">
																...
																<button
																	...
																	className="text-sm bg-red-600 py-2 px-2 rounded-xl text-white hover:bg-red-800 hover:cursor-pointer"
																>
																...
																{service.options.map((option) => {
																	...
																	return (
																		<button
																			...
																			onMouseEnter={() =>
																				setRandomHoverColor(
																					`duration-${item.serviceId}-${index}-${option.duration}`
																				)
																			}
																			onMouseLeave={() =>
																				clearHoverColor(
																			...
																			className={[
																				...
																				active
																					? "bg-[#405d3f] text-white border-[#405d3f]"
																					: "bg-white text-gray-700 border-[#82a687]/20",
																			].join(" ")}
																			style={
																				!active
																					? {
									...
									<div className="rounded-3xl border border-white/60 bg-white/80 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
										<p className={`text-base text-gray-800 underline ${playfairBold.className}`}>
											Add another service
										</p>
											...
											{services
												...
												.map((service) => (
													<button
														...
														onMouseEnter={() =>
															setRandomHoverColor(`service-${service.id}`)
														}
														onMouseLeave={() =>
															clearHoverColor(`service-${service.id}`)
														}
														className="rounded-2xl border border-[#82a687]/20 bg-white px-4 py-4 text-left transition cursor-pointer"
														style={{
															backgroundColor:
																hoverColors[`service-${service.id}`],
														}}
													>
														...
														<p className={`mt-1 text-xs text-gray-500 ${playfairRegular.className}`}>
															From {formatCurrency(service.options[0]?.price ?? 0)}
														</p>
													</button>
												))}
										</div>
									...
									<div className="rounded-3xl border border-white/60 bg-white/80 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
										...
										<button
											...
											onClick={() => goToStep(2)}
											className={`mt-5 w-full rounded-full bg-[#405d3f] hover:bg-[#2e4c2d] transition text-white py-3 px-6 hover:cursor-pointer  disabled:opacity-50 disabled:cursor-not-allowed ${playfairBold.className}`}
										>
